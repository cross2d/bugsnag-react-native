...
function addFrameworkAndSearchPath() {
    ...
    var frameworkPath = path.join(__dirname,'../node_modules/@cross2d/react-native-alipay/ios/SDK/AlipaySDK.framework');
    ...
    project.parse(function (error) {
        if (error) {
            ...
            addSearchPaths(project,'"$(SRCROOT)/../node_modules/@cross2d/react-native-alipay/ios/SDK"');
 ...
 * @param key
 * @param value
 * @param overwrite 是否覆盖老记录 默认为 false
 */
function addPlist(key,value,overwrite=false) {
    var parsedInfoPlist = plist.parse(plistContents);
    if(overwrite){
        if(parsedInfoPlist.hasOwnProperty(key)){
            return;
        }
    }
    parsedInfoPlist[key]=value;
    plistContents = plist.build(parsedInfoPlist);
    fs.writeFileSync(plistPath, plistContents);
}

function androidManifestAddChildToApplication(value){
    var manifest = new AndroidManifest().readFile(manifestPath)
    manifest.$('application').append(value)
    manifest.writeFile(manifestPath);
}


const BugsnagAPIKey = 'd6da287cb8beb0b534cfe43da5798095'

androidManifestAddChildToApplication('<meta-data android:name="com.bugsnag.android.API_KEY" android:value="'+BugsnagAPIKey+'"/>')
addPlist("BugsnagAPIKey",BugsnagAPIKey)
